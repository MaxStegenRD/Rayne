...
function(INTERNAL_rayne_stage_android_asset _SOURCE _DESTINATION)
    ...
    if(IS_DIRECTORY "${_SOURCE}")
        file(GLOB_RECURSE _RAYNE_ANDROID_ASSET_INPUTS LIST_DIRECTORIES false CONFIGURE_DEPENDS "${_SOURCE}/*")
        file(REMOVE_RECURSE "${_DESTINATION}")
        file(COPY "${_SOURCE}" DESTINATION "${_DESTINATION_DIRECTORY}")
    elseif(EXISTS "${_SOURCE}")
        ...
        message(FATAL_ERROR "Android asset source does not exist: ${_SOURCE}")
    endif()
endfunction()
...
macro(rayne_use_modules _TARGET _MODULES)
    foreach(_MODULE ${_MODULES})
        ...
        if(ANDROID)
            ...
            foreach(_RESOURCE ${${_MODULE}_MODULE_RESOURCES})
                INTERNAL_rayne_stage_android_asset("${_MODULE_RESOURCE_SOURCE_DIRECTORY}/${_RESOURCE}" "${android-assets-dir}/Resources/Modules/${_MODULE}/${_RESOURCE}")
            endforeach()
        endif()
    endforeach()
endmacro()
    ...
    foreach(_RESOURCE ${_RESOURCES})
        if(ANDROID)
            ...
            if(IS_DIRECTORY "${_RESOURCE_SOURCE}")
                file(GLOB_RECURSE _RAYNE_ANDROID_RESOURCE_INPUTS LIST_DIRECTORIES false CONFIGURE_DEPENDS "${_RESOURCE_SOURCE}/*")
                file(REMOVE_RECURSE "${_RESOURCE_DESTINATION}")
                execute_process(COMMAND ${Python_EXECUTABLE} "${DIR_OF_RAYNE_CMAKE}/../Tools/ResourcePacker/pack.py" "${_RESOURCE_SOURCE}" "${_RESOURCE_DESTINATION}" android ${_ADDITIONAL_PACK_PARAMS} COMMAND_ERROR_IS_FATAL ANY)
            else()
                INTERNAL_rayne_stage_android_asset("${_RESOURCE_SOURCE}" "${_RESOURCE_DESTINATION}")
            endif()
        elseif(APPLE)
