...
namespace RN
{
	...
	OculusAudioSystem::OculusAudioSystem(uint32 sampleRate, uint32 frameSize) :
		_frameSize(frameSize), _sampleRate(sampleRate)
	{
	}
	...
	OculusAudioSystemRtAudio::OculusAudioSystemRtAudio(uint32 sampleRate, uint32 frameSize) :
		OculusAudioSystem(sampleRate, frameSize), _internals(new OculusAudioSystemRtAudioInternals()), _outputDevice(nullptr), _inputDevice(nullptr)
	{
	}
