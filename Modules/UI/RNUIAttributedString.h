...
namespace RN
{
	namespace UI
	{
		...
		public:
			UIAPI TextAttributes(Font *font, float fontSize, RN::Color color, TextAlignment alignment = TextAlignmentLeft, TextWrapMode wrapMode = TextWrapModeWord, float kerning = 0.0f) :
				_font(font->Retain()), _color(color), _alignment(alignment), _wrapMode(wrapMode), _fontSize(fontSize), _kerning(kerning), _range(0, 0) {}
