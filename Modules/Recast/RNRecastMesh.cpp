...
namespace RN
{
	...
	RecastMesh::Configuration::Configuration() :
		cellSize(0.1f), cellHeight(0.1f), agentHeight(1.8f), agentRadius(0.2f), agentMaxClimb(0.4f), agentMaxSlope(45.0f), regionMinSize(2.0f), regionMergeSize(20.0f), edgeMaxLength(12.0f), edgeMaxError(0.3f), detailSampleDist(6.0f), detailSampleMaxError(1.0f), maxVertsPerPolygon(6)
	{
		float cellSize;
		float cellHeight;
	...
	RecastMesh::RecastMesh(Model *model, const Configuration &configuration) :
		_isDirty(false), _navMesh(nullptr), _navMeshQuery(nullptr)
	{
		//TODO: Fix this
	...
	RecastMesh::RecastMesh(Array *meshes, const Configuration &configuration) :
		_isDirty(false), _navMesh(nullptr), _navMeshQuery(nullptr)
	{
		_configuration = configuration;
	...
	RecastMesh::RecastMesh(Mesh *mesh, const Configuration &configuration) :
		_isDirty(false), _navMesh(nullptr), _navMeshQuery(nullptr)
	{
		_configuration = configuration;
		AddMesh(mesh);
