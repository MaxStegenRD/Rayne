...
/*		_colorFormat = D3D12ImageFormatFromTextureFormat(descriptor.colorFormat);
	...
	void VulkanFramebuffer::PrepareAsRendertargetForFrame(const RenderFrame &renderFrame, const VulkanRenderPass *renderPass)
	{
		...
		VkDevice device = _renderer->GetVulkanDevice()->GetDevice();
		newVariant.renderPass = _renderer->GetVulkanRenderPass(renderFrame, renderPass);
