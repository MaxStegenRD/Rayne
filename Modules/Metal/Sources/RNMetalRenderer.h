...
namespace RN
{
	...
	protected:
		...
		void RenderDrawable(MetalDrawable *drawable, uint32 instanceCount, uint8 renderPacketSlot);
		void RenderAPIRenderPass(const MetalRenderPass &renderPass, uint8 renderPacketSlot);
		void FillUniformBuffer(Shader::ArgumentBuffer *argument, MetalUniformBufferReference *uniformBufferReference, MetalDrawable *drawable, const Material::Properties &materialProperties, uint8 renderPacketSlot);
